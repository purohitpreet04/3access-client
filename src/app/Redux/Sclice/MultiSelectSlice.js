...
const sideselectSlice = createSlice({
    ...
    reducers: {
        ...
        setSelectedData: (state, action) => {
            ...
            const comdata = JSON.parse(JSON.stringify(state.comData))
    ...
    extraReducers: (builder) => {
        builder
            .addCase(addSelectedValue.fulfilled, (state, action) => {
                const selectedIds = new Set(action.payload.comp.split(',')); // Create a set of selected IDs from the payload
                const comdata = JSON.parse(JSON.stringify(state.comData))
                state.selectedValues = comdata.filter((item) => selectedIds.has(item?._id));
